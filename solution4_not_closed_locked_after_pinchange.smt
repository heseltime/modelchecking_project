...
; bug: The door is not closed and locked after changing the stored PIN.
;   assumption: locked 0 is meant. Anded with is_closed, though this might be redundant.
;           "change": the pin is now different (entering the same does not count)
(define-fun not_closed_locked_after_pinchange ((i Int)) Bool (=>
    ; pin is changed in state unlocked
    ((_ is unlocked) (keypadstate i)) 
    (and    ((_ is wrongpin) (keypresses i))
        (= (keypadstate (+ i 1)) (locked 0))
        (not ((_ is open) (keypadstate (+ i 1))))
    )    
))
...
(assert (forall ((i Int)) (=> (and
    ...
    (and
        ...
        ;
        (not_closed_locked_after_pinchange i)
        ;
        ;
        ;
    ))))
